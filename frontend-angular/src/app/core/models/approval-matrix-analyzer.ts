/**
 * Ma trận phê duyệt — phân tích xung đột & độ phủ. Port của
 * webapp/src/data/approvalMatrixAnalyzer.ts (D17 Angular migration).
 *
 * Vì hệ FAIL-CLOSED (không luật khớp = không có người xử lý), việc phát hiện luật
 * bị che khuất (shadow) hoặc slot thiếu fallback là cơ chế ĐÚNG-SAI, không chỉ
 * tiện lợi. Analyzer thuần logic, không UI.
 */

import type { ApprovalRule, SlotCode } from './approval-matrix';
import { APPROVAL_SLOTS } from './approval-slot-catalog';
