/**
 * Ma trận phê duyệt — động cơ điều kiện (condition engine). Port của
 * webapp/src/data/approvalConditions.ts (EPIC06, D17 Angular migration).
 *
 * Cây điều kiện AND/OR tổng quát + tập toán tử, đánh giá trên một context phẳng
 * (Record<string, unknown>). Không phụ thuộc registry — registry
 * (approval-variable-registry.ts) cấp nhãn đẹp qua DescribeHelpers.
 */
...
export function evaluateLeaf(leaf: ConditionLeaf, ctx: EvalContext): boolean {
  ...
  switch (leaf.operator) {
    ...
    case 'between': {
      const a = toNum(actual);
      const lo = toNum(leaf.value);
      const hi = toNum(leaf.valueTo);
      return !Number.isNaN(a) && !Number.isNaN(lo) && !Number.isNaN(hi) && a >= lo && a <= hi;
    }
    case 'in': {
 ...
 * (wildcard) → true. AND = mọi nhánh đúng; OR = ít nhất một nhánh đúng.
 */
export function evaluateConditionTree(node: ConditionNode, ctx: EvalContext): boolean {
...
export function describeLeaf(leaf: ConditionLeaf, h?: DescribeHelpers): string {
  ...
  if (meta?.arity === 'range') {
    return `${field} ${phrase} ${fmtValue(leaf.field, leaf.value, h)} – ${fmtValue(leaf.field, leaf.valueTo, h)}`;
  }
 ...
 */
export function describeConditionTree(node: ConditionNode, h?: DescribeHelpers, depth = 0): string {
  if (node.kind === 'condition') return describeLeaf(node, h);
  if (node.items.length === 0) return '(bất kỳ)';
  const sep = node.logic === 'AND' ? ' VÀ ' : ' HOẶC ';
  const inner = node.items.map((it) => describeConditionTree(it, h, depth + 1)).join(sep);
...
export function group(logic: 'AND' | 'OR', items: ConditionNode[]): ConditionGroup {
