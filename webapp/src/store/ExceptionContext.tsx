...
interface ExceptionCtxValue {
  ...
  /** Yêu cầu Chi tiết đang chờ duyệt cho một hồ sơ (nếu có). */
  ...
  /** Toàn bộ yêu cầu Chi tiết (mọi trạng thái) của một hồ sơ — hiển thị trên timeline. */
  ...
  /** Duyệt Chi tiết (APPROVE_EXCEPTION): chuyển sang `approved` — CHƯA đổi luồng hồ sơ. */
  approveException: (id: string, actor: string, note?: string) => void;
  /** Từ chối Chi tiết. */
  rejectException: (id: string, actor: string, note: string) => void;
  /** Áp dụng Chi tiết đã duyệt (APPLY_EXCEPTION): `approved` → `applied` + nhảy bước thật. */
...
export function useExceptions(): ExceptionCtxValue {
  ...
  if (!ctx)
    throw new Error("useExceptions must be used within ExceptionProvider");
  return ctx;
}
 ...
 * quyết định Chi tiết (ở đây) khỏi việc thay đổi luồng hồ sơ (DossierContext).
 */
export function ExceptionProvider({ children }: { children: ReactNode }) {
  ...
  const forDossier = (hoSoId: string) =>
    requests.filter((r) => r.hoSoId === hoSoId);
  ...
  const approveException: ExceptionCtxValue["approveException"] = (
    id,
    actor,
    note,
  ) => {
  ...
  const rejectException: ExceptionCtxValue["rejectException"] = (
    id,
    actor,
    note,
  ) => {
  ...
  // APPLY: thực thi Chi tiết ĐÃ duyệt vào luồng (Dynamic Routing) — chỉ chạy khi đã `approved`.
  const applyException: ExceptionCtxValue["applyException"] = (id, actor) => {
    ...
    applyExceptionSkip(
      ...
      req.fromStepIndex,
      req.toStepIndex,
      `[Chi tiết] ${req.decisionNote ?? ""} — duyệt bởi ${req.decidedBy ?? "—"}, áp dụng bởi ${actor}`.trim(),
      actor,
    );
  };
