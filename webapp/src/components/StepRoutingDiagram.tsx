...
/** Nhánh Chi tiết (nét đứt) — không thuộc luồng chuẩn, cần duyệt riêng. */
...
interface Props {
  ...
  /** Nhánh Chi tiết khả dụng ở bước này (vẽ nét đứt, tách khỏi luồng chuẩn). */
...
export default function StepRoutingDiagram({
  ...
  if (!branches.length) {
    return (
      <Empty
        description="Bước này chưa cấu hình định tuyến."
    ...
    <div>
      ...
      >
        {branches.map((b) => {
          ...
          const dest =
            b.toStepIndex !== undefined ? steps[b.toStepIndex] : undefined;
          const names =
            showApprovers && dest ? approverNames(dest.vaiTroCodes) : [];
          return (
            <div
              key={b.outcome}
              style={{
            ...
            >
              <Tag
                color={meta.tag}
                icon={meta.icon}
                style={{ marginTop: 2, whiteSpace: "normal" }}
              >
                {b.label}
              </Tag>
      ...
      <Text type="secondary" style={{ fontSize: 11 }}>
        Nhánh liền = Routing Matrix (cùng nguồn nút “Xử lý”); nhánh nét đứt =
        Chi tiết có kiểm soát.
      </Text>
    </div>
  );
}
