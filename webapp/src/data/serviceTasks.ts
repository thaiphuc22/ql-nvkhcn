import { seedIntegrations } from './camundaOps'
import {
  seedMappingConfigs,
...
// Domain model cho module "Cấu hình Service Task / Tác vụ hệ thống".
//
// File này là nguồn mock duy nhất cho Đợt 1: registry type, definition, version,
// binding BPMN, execution log và audit. Model đặt theo hướng backend để Đợt 2
// có thể bọc bằng Context, sau này thay bằng API thật mà không đổi nhiều UI.
...
export type ServiceTaskCategory =
  | 'communication'
  | 'integration'
  | 'data'
  | 'document'
...
export const SERVICE_TASK_STATUS_META: Record<
  ServiceTaskDefinitionStatus,
...
export type ServiceTaskConfigVersionStatus =
  | 'DRAFT'
  | 'READY'
  | 'ACTIVE'
  | 'ARCHIVED'
...
export type ServiceTaskFailurePolicy =
  | 'CREATE_INCIDENT'
  | 'FAIL_PROCESS'
  | 'CONTINUE'
  | 'COMPENSATE'
...
export type ServiceTaskOutputTarget =
  | 'variables'
  | 'dossier'
  | 'integrationRef'
  | 'executionMetadata'
...
/**
 * Metadata service task đọc từ BPMN/process. Đợt 4 có thể chuyển sang file
 * `serviceTaskReconcile.ts`; giữ ở đây tạm thời để seed binding/log có điểm bám.
 */
...
export const SERVICE_TASK_EXECUTION_STATUS_META: Record<
  ServiceTaskExecutionStatus,
  { label: string; color: string }
> = {
...
export function valueAtJsonPath(
...
function validateExecutionConfig(
  ...
  if (config.typeCode === 'SEND_NOTIFICATION') {
    ...
    const err = validateExpression(config.recipientExpression, [
      'variables',
      'dossier',
      'initiator',
      'assignee',
  ...
  if (config.typeCode === 'CALL_API') {
    ...
    if (config.idempotencyKeyExpression) {
      const err = validateExpression(config.idempotencyKeyExpression, [
        'variables',
        'dossier',
        'task',
        'system',
  ...
  if (config.typeCode === 'UPDATE_DOSSIER') {
    ...
    config.updates.forEach((u) => {
      ...
      const err = validateExpression(u.valueExpression, [
        'variables',
        'dossier',
        'form',
        'system',
  ...
  if (config.typeCode === 'GENERATE_DOCUMENT') {
    ...
    if (config.outputFolderExpression) {
      const err = validateExpression(config.outputFolderExpression, [
        'variables',
        'dossier',
        'task',
        'system',
...
export function validateServiceTaskConfig(
  ...
  const context: Required<ServiceTaskValidationContext> = {
    ...
    dossierFieldWhitelist:
      validationContext.dossierFieldWhitelist ?? DOSSIER_OUTPUT_FIELD_WHITELIST,
  }
  const errors: string[] = []
    ...
    outputMapping: [
      {
        ...
        target: 'executionMetadata',
        targetPath: 'notificationMessageId',
      },
    ],
    errorPolicy: {
  ...
  },
  {
    ...
    inputMapping: [
      ...
      },
      {
        id: 'in-sap-2',
        target: 'tongDuToan',
        expression: '${variables.totalBudget}',
        source: 'variables',
        required: true,
      },
      {
    ...
    outputMapping: [
      {
        ...
        target: 'variables',
        targetPath: 'sapBudgetRequestId',
      },
      {
        id: 'out-sap-2',
        sourcePath: '$.status',
        target: 'variables',
        targetPath: 'sapSyncStatus',
      },
      {
        id: 'out-sap-3',
        sourcePath: '$.externalId',
        target: 'dossier',
        targetPath: 'maDongBoNgoai',
      },
    ],
    errorPolicy: {
      ...DEFAULT_ERROR_POLICY,
    ...
    outputMapping: [
      {
        ...
        sourcePath: '$.updated',
        target: 'executionMetadata',
        targetPath: 'dossierUpdated',
      },
    ],
    errorPolicy: {
      ...DEFAULT_ERROR_POLICY,
      maxRetry: 0,
      onFailure: 'FAIL_PROCESS',
    },
    status: 'READY',
    ...
    outputMapping: [
      {
        ...
        target: 'dossier',
        targetPath: 'documentIds',
      },
      {
        id: 'out-doc-2',
        sourcePath: '$.fileName',
        target: 'variables',
        targetPath: 'acceptanceDecisionFileName',
      },
    ],
    errorPolicy: {
      ...DEFAULT_ERROR_POLICY,
    ...
    outputMapping: [
      {
        ...
        sourcePath: '$.decision',
        target: 'variables',
        targetPath: 'rd02RoutingDecision',
      },
      {
        id: 'out-dmn-2',
        sourcePath: '$.reason',
        target: 'variables',
        targetPath: 'rd02RoutingReason',
      },
    ],
    errorPolicy: {
      ...DEFAULT_ERROR_POLICY,
...
export const seedServiceTaskExecutionLogs: ServiceTaskExecutionLog[] = [
  ...
  },
  {
    ...
    responseSummary: {
      documentId: 'doc-035-qd-cong-nhan',
      fileName: 'QD-cong-nhan-HS-2026-035.pdf',
    },
...
export const sampleServiceTaskContext: ServiceTaskSampleContext = {
  ...
  system: {
    today: '2026-07-09',
    acceptanceDecisionTemplateCode: 'qd-cong-nhan',
  },
  previousOutput: {},
}
