// Chi tiết có kiểm soát (Controlled Exception Handling) — mock/prototype F0.
// Tham chiếu: docs/research/controlled-exception-handling.md, action-availability-model.md.
// Đây là lớp riêng biệt với luồng phê duyệt chuẩn (DossierStep): một yêu cầu Chi tiết
...
export const EXCEPTION_STATUS_LABEL: Record<
  ExceptionRequestStatus,
  { label: string; color: string }
> = {
  pending: { label: "Chờ duyệt Chi tiết", color: "gold" },
  approved: { label: "Đã duyệt — chờ áp dụng", color: "blue" },
  rejected: { label: "Chi tiết bị từ chối", color: "red" },
  applied: { label: "Đã áp dụng Chi tiết", color: "purple" },
};
...
export interface ExceptionRequest {
  ...
  /** Bước hiện tại của hồ sơ tại thời điểm xin Chi tiết. */
...
// Ai được duyệt Chi tiết, có bắt buộc căn cứ không, tối đa mấy lần/hồ sơ — đã chuyển
