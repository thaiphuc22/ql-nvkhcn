...
export function getUserRoleCodes(user: AppUser | null | undefined): string[] {
  ...
  return [
    ...new Set(
      user.vaiTro.flatMap((label) => ROLE_LABEL_TO_CODES[label] ?? []),
    ),
  ];
}
 ...
 * mới được xin Chi tiết trên bước đó — không mở cho toàn bộ người xem hồ sơ.
 */
 ...
 * yêu cầu Chi tiết (không phải vai trò của bước gốc) mới được duyệt/từ chối — tách
...
/**
 * Quyền APPLY_EXCEPTION: người xử lý bước gốc (hoặc admin) THỰC THI Chi tiết ĐÃ được
...
/**
 * Quyền VIEW_EXCEPTION_AUDIT: xem lịch sử Chi tiết của hồ sơ. Mở cho người tham gia hồ sơ
 * (người xử lý bước hiện tại, vai trò được duyệt Chi tiết) và admin — không mở cho người
 * ngoài. Audit Chi tiết luôn hiển thị công khai với các bên liên quan (không "âm thầm").
 */
