...
export const PERMISSION_LABEL: Record<string, string> = {
  ...
  REQUEST_EXCEPTION: "Xin Chi tiết",
...
export function resolveActionAvailability(
      ...
      .filter(
        (p) =>
          ...
          (p.taskDefinitionKey == null ||
            p.taskDefinitionKey === ctx.taskDefinitionKey),
      )
  ...
  if (!matched) {
    return {
      ...
      enabled: false,
      reasons: [
        "Không có luật cho phép action này ở surface/quy trình/trạng thái hiện tại.",
      ],
    };
  }
  ...
  if (ctx.isAdmin) {
    return {
      ...
      visible: true,
      enabled: true,
      reasons: ["Admin — bỏ qua kiểm tra vai trò/quyền."],
    };
  }
...
export function availabilityConditionText(p: ActionAvailabilityPolicy): string {
  const parts = [
    ...
    p.allowedRoleCodes.length
      ? `role in [${p.allowedRoleCodes.join(", ")}]`
      : "role = any",
  ];
...
export const SEED_AVAIL_AUDIT: ActionAvailabilityAuditEntry[] = [
  {
    ...
    detail: "Tạo luật hiển thị nút SUBMIT cho hồ sơ draft.",
  },
  {
    id: "ava-ap01-2",
    policyId: "AP-01",
    ...
    detail: "Thêm vai trò NNC vào danh sách được phép.",
  },
  {
    id: "ava-ap03-1",
    policyId: "AP-03",
    ...
    detail: "Tạo luật ADD_COMMENT (mọi trạng thái).",
  },
  {
    id: "ava-ap04-1",
    policyId: "AP-04",
    ...
    detail: "Tạo luật DOWNLOAD_DOSSIER.",
  },
  {
    id: "ava-ap05-1",
    policyId: "AP-05",
    ...
    version: 1,
    actor: "Quản trị hệ thống",
    timestamp: "2026-06-05 12:00",
    detail: "Tạo luật VIEW_HISTORY.",
  },
];
