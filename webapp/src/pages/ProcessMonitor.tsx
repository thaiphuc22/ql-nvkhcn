...
export default function ProcessMonitor() {
  ...
  const stats = useMemo(() => {
    ...
    const incident = seedInstances.filter(
      (i) => i.trangThai === "incident",
    ).length;
    const done = seedInstances.filter(
      (i) => i.trangThai === "completed",
    ).length;
      ...
      <FilterBar
        ...
        selects={[
          ...
          },
          {
            ...
            onChange: setFState,
            width: 170,
            options: (Object.keys(INSTANCE_STATE) as InstanceState[]).map(
              (k) => ({ value: k, label: INSTANCE_STATE[k].label }),
            ),
          },
        ]}
        right={
      ...
      >
        {detail && (
          <div style={{ marginTop: 8 }}>
            ...
            <Row gutter={[12, 10]}>
              <Field
                label="Mã hồ sơ (correlation key)"
                value={<Text strong>{detail.maHoSo}</Text>}
              />
                ...
                value={
                  detail.hanXuLy ? (
                    <Tooltip title={detail.quaHan ? "Đã quá hạn" : "Trong hạn"}>
                      <Tag color={detail.quaHan ? "error" : "default"}>
                        {detail.hanXuLy}
                      </Tag>
                    </Tooltip>
                  ) : (
            ...
            >
              Nguyên tắc D3: process variables chỉ giữ <Text code>maHoSo</Text>{" "}
              + biến điều khiển (<Text code>cap</Text>,{" "}
              <Text code>ketQuaThamDinh</Text>…). Nội dung hồ sơ mở ở màn nghiệp
              vụ.
            </Paragraph>
          </div>
        )}
