...
export default function IntegrationStatus() {
  const stats = useMemo(() => {
    ...
    const healthy = seedIntegrations.filter(
      (s) => s.trangThai === "healthy",
    ).length;
  ...
  return (
    <div>
      ...
      <Row gutter={14} style={{ marginBottom: 18 }}>
        <Col xs={12} md={6}>
          <StatCard title="Hệ tích hợp" value={stats.total} />
        </Col>
        <Col xs={12} md={6}>
          <StatCard title="Đang khoẻ" value={stats.healthy} color="#17935a" />
        </Col>
        <Col xs={12} md={6}>
          <StatCard title="Lỗi 24h" value={stats.errors} color="#cf1322" />
        </Col>
        <Col xs={12} md={6}>
          <StatCard
      ...
      {/* <Paragraph type="secondary" style={{ fontSize: 12 }}>
        ...
        <Link to="/nhat-ky">Nhật ký → tab "Nhật ký tích hợp"</Link>. Camunda
        điều phối gọi hệ ngoài qua service task; logic tích hợp nằm ở job
    ...
    <Card size="small" style={{ height: "100%" }}>
      <Space direction="vertical" size={8} style={{ width: "100%" }}>
        ...
        <Row gutter={8} style={{ marginTop: 2 }}>
          <Metric
            label="Bản ghi 24h"
            value={s.banGhi24h.toLocaleString("vi-VN")}
          />
          <Metric label="Lỗi 24h" value={s.loi24h} danger={s.loi24h > 0} />
          <Metric
            label="Độ trễ"
            value={s.doTreMs ? `${s.doTreMs}ms` : "—"}
            danger={s.doTreMs > 1000}
          />
      ...
      >
        ...
        {danger && (
          <ExclamationCircleFilled
            style={{ fontSize: 11, marginInlineEnd: 3 }}
          />
        )}
        {value}
      </div>
