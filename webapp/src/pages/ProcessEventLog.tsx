  ...
  return (
    <div>
      <Row gutter={14} style={{ marginBottom: 18 }}>
        <Col xs={8}>
          <StatCard title="Sự kiện" value={stats.total} />
        </Col>
        <Col xs={8}>
          <StatCard
            title="Gọi hệ ngoài"
        ...
        <Col xs={8}>
          <StatCard title="Sự cố" value={stats.incidents} color="#cf1322" />
        </Col>
      </Row>
      ...
      <FilterBar
        selects={[
          ...
          },
          {
            ...
            onChange: setFType,
            width: 220,
            options: (Object.keys(EVENT_META) as EventType[]).map((k) => ({
              value: k,
      ...
      >
        {items.length ? (
          ...
          <Empty description="Không có sự kiện khớp bộ lọc." />
        )}
      </div>

      <Text
        type="secondary"
        style={{ fontSize: 12, display: "block", marginTop: 14 }}
      >
        Nguồn: lịch sử Zeebe/Operate (mock). Đây là audit{" "}
        <Text strong>lớp điều phối</Text> (luồng đi bước nào, rẽ nhánh, gọi hệ
        ngoài, sự cố). Audit <Text strong>nội dung</Text> (ai sửa dữ liệu gì
        trong hồ sơ) nằm ở ứng dụng nghiệp vụ — REQ-ENG-014.
      </Text>
    </div>
  );
}
...
function IntegrationLog() {
  ...
  const systems = useMemo(
    () => Array.from(new Set(seedJobRuns.map((j) => j.he))).sort(),
    [],
  );
    ...
    },
    {
      ...
      render: (v: JobOutcome, r) => (
        <Space direction="vertical" size={2}>
          <StatusTag
            color={JOB_OUTCOME[v].color}
            label={JOB_OUTCOME[v].label}
          />
  ...
  return (
    <div>
      <Row gutter={14} style={{ marginBottom: 18 }}>
        <Col xs={8}>
          <StatCard title="Lần chạy job" value={stats.total} />
        </Col>
        <Col xs={8}>
          <StatCard title="Đang thử lại" value={stats.retry} color="#b06f00" />
        </Col>
        <Col xs={8}>
          <StatCard title="Thất bại" value={stats.failed} color="#cf1322" />
        </Col>
      </Row>
      ...
      <FilterBar
        selects={[
          ...
          },
          {
            ...
            onChange: setFKetQua,
            width: 170,
            options: (Object.keys(JOB_OUTCOME) as JobOutcome[]).map((k) => ({
              value: k,
              label: JOB_OUTCOME[k].label,
            })),
          },
        ]}
        right={
          <Text type="secondary">
            {rows.length}/{seedJobRuns.length} lần chạy
          </Text>
        }
      />
      ...
      <Text
        ...
        style={{ fontSize: 12, display: "block", marginTop: 14 }}
      >
        Mỗi dòng là một lần <Text strong>job worker</Text> chạy — Camunda giao
        việc qua service task, worker của app gọi hệ ngoài
        (docs/arch/camunda-design.md mục 5). Xem sức khoẻ tổng thể từng hệ ở màn
        <Text strong> Trạng thái Tích hợp</Text>.
      </Text>
    </div>
  );
}
