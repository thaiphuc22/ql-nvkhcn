...
export default function ServiceTaskConfig() {
  ...
  const [typeFilter, setTypeFilter] = useState<
    ServiceTaskTypeCode | undefined
  >();
  const [statusFilter, setStatusFilter] = useState<
    ServiceTaskDefinitionStatus | undefined
  >();
  ...
  const [logProcessFilter, setLogProcessFilter] = useState<
    string | undefined
  >();
  const [logDefinitionFilter, setLogDefinitionFilter] = useState<
    string | undefined
  >();
  const [logStatusFilter, setLogStatusFilter] = useState<
    ServiceTaskExecutionStatus | undefined
  >();
  const [logIncidentFilter, setLogIncidentFilter] = useState<
    boolean | undefined
  >();
  const [selectedLogId, setSelectedLogId] = useState<string | undefined>();
  const [auditDefinitionFilter, setAuditDefinitionFilter] = useState<
    string | undefined
  >();

  const rows = useMemo<DefinitionRow[]>(() => {
    return serviceTasks.definitions.map((definition) => {
      const type = serviceTasks.types.find(
        (item) => item.code === definition.typeCode,
      );
      const versions = serviceTasks.getVersions(definition.id);
      const latest = latestVersion(versions);
      const bindings = serviceTasks.bindings.filter(
        (binding) =>
          binding.serviceTaskDefinitionId === definition.id &&
          binding.bindingStatus === "ACTIVE",
      );
      const logs = serviceTasks.executionLogs.filter(
        (log) => log.serviceTaskDefinitionId === definition.id,
      );
      const successCount = logs.filter(
        (log) => log.status === "SUCCESS",
      ).length;
      const incidentCount = logs.filter(
        (log) => log.status === "FAILED" && log.incidentId,
      ).length;
      return {
        ...definition,
        ...
        processCodes: [
          ...new Set(bindings.map((binding) => binding.processCode)),
        ],
  ...
  const filteredRows = rows.filter((row) => {
    ...
    if (processFilter && !row.processCodes.includes(processFilter))
      return false;
    if (connectorFilter && row.connectorKey !== connectorFilter) return false;
    if (
      incidentFilter !== undefined &&
      row.incidentCount > 0 !== incidentFilter
    )
      return false;
    return true;
  });
  ...
  const reconcileProcesses = useMemo(
    () => reconcilableServiceTaskProcesses(),
    [],
  );
  ...
  const openIncidents = serviceTasks.executionLogs.filter(
    (log) => log.status === "FAILED" && log.incidentId,
  ).length;
  ...
  const topProcesses = useMemo(() => {
    ...
    serviceTasks.bindings
      ...
      .forEach((b) =>
        map.set(b.processCode, (map.get(b.processCode) ?? 0) + 1),
      );
    return [...map.entries()]
      .sort((a, b) => b[1] - a[1])
  ...
  const activeFilters: { key: string; label: string; onClear: () => void }[] = [
    ...(typeFilter
      ? [
          {
            key: "type",
            label: `Loại: ${serviceTasks.types.find((t) => t.code === typeFilter)?.name ?? typeFilter}`,
    ...
    ...(statusFilter
      ? [
          {
            key: "status",
            label: `Trạng thái: ${SERVICE_TASK_STATUS_META[statusFilter].label}`,
    ...
    ...(processFilter
      ? [
          {
            key: "process",
            label: `Quy trình: ${processFilter}`,
            onClear: () => setProcessFilter(undefined),
          },
        ]
      : []),
    ...(connectorFilter
      ? [
          {
            key: "connector",
            label: `Connector: ${connectorFilter}`,
            onClear: () => setConnectorFilter(undefined),
          },
        ]
      : []),
    ...(incidentFilter !== undefined
      ? [
          {
            key: "incident",
            label: `Incident: ${incidentFilter ? "Có" : "Không"}`,
            onClear: () => setIncidentFilter(undefined),
          },
        ]
      : []),
  ];
  ...
  const connectorOptions = [
    ...new Set(rows.map((row) => row.connectorKey).filter(Boolean) as string[]),
  ].map((value) => ({ label: value, value }));
  ...
  const selectedLog = selectedLogId
    ...
    : undefined;
  const selectedLogDefinition = selectedLog
    ? serviceTasks.getDefinition(selectedLog.serviceTaskDefinitionId)
    : undefined;
  const selectedLogVersion = selectedLog
    ? serviceTasks.getVersion(
        selectedLog.serviceTaskDefinitionId,
        selectedLog.configVersionNo,
      )
    : undefined;
  const filteredExecutionLogs = serviceTasks.executionLogs.filter((log) => {
    ...
    if (
      logDefinitionFilter &&
      log.serviceTaskDefinitionId !== logDefinitionFilter
    )
      return false;
    if (logStatusFilter && log.status !== logStatusFilter) return false;
    if (
      logIncidentFilter !== undefined &&
      Boolean(log.incidentId) !== logIncidentFilter
    )
      return false;
    return true;
  });
  const versionRows = serviceTasks.versions.filter((version) =>
    auditDefinitionFilter
      ? version.serviceTaskDefinitionId === auditDefinitionFilter
      : true,
  );
  const auditEntityIds = new Set([
    ...(auditDefinitionFilter
      ? [auditDefinitionFilter]
      : serviceTasks.definitions.map((definition) => definition.id)),
    ...versionRows.map((version) => version.id),
    ...serviceTasks.bindings
      .filter(
        (binding) =>
          !auditDefinitionFilter ||
          binding.serviceTaskDefinitionId === auditDefinitionFilter,
      )
    ...
    ...serviceTasks.executionLogs
      .filter(
        (log) =>
          !auditDefinitionFilter ||
          log.serviceTaskDefinitionId === auditDefinitionFilter,
      )
      .map((log) => log.id),
  ]);
  ...
  const validate = (definition: ServiceTaskDefinition) => {
    ...
    const result = serviceTasks.validateVersion(
      definition.id,
      version.versionNo,
      actor,
    );
  ...
  const activate = (definition: ServiceTaskDefinition) => {
    ...
    const result = serviceTasks.activateVersion(
      definition.id,
      version.versionNo,
      actor,
      "Active từ màn cấu hình.",
    );
    if (result.valid)
      message.success(`Đã active version ${version.versionNo}.`);
  ...
  const columns: ColumnsType<DefinitionRow> = [
    ...
    },
    {
      ...
      render: (_, row) => {
        ...
        return (
          <Space size={2} wrap>
            {visible.map((code) => (
              <Tag key={code} style={{ fontSize: 11, margin: 0 }}>
                {code}
              </Tag>
            ))}
            {rest > 0 && (
        ...
        return (
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <Progress
              ...
              showInfo={false}
              strokeColor={
                pct >= 90 ? "#1677ff" : pct >= 70 ? "#fa8c16" : "#cf1322"
              }
    ...
    },
    {
      ...
      render: (value) =>
        value ? <Tag color="error">{value}</Tag> : <Tag>0</Tag>,
    },
    {
      title: "Cập nhật",
      dataIndex: "updatedAt",
      ...
      <Row gutter={[12, 12]} style={{ marginBottom: 16 }}>
              ...
              trends.total !== undefined ? (
                <span
                  style={{
                    fontSize: 12,
                    color:
                      trends.total.direction === "up" ? "#1677ff" : "#cf1322",
                  }}
                >
        ...
        <Col xs={24} sm={12} md={6}>
          <StatCard
            ...
            color="#1677ff"
            suffix={
              trends.active !== undefined ? (
                <span
                  style={{
                    fontSize: 12,
                    color:
                      trends.active.direction === "up" ? "#1677ff" : "#cf1322",
                  }}
                >
        ...
        <Col xs={24} sm={12} md={6}>
          <StatCard
            ...
            suffix={
              trends.missingBinding !== undefined ? (
                <span
                  style={{
                    fontSize: 12,
                    color:
                      trends.missingBinding.direction === "down"
                        ? "#389e0d"
                        : "#cf1322",
                  }}
                >
                  {trends.missingBinding.direction === "down" ? "↓" : "↑"}{" "}
                  {trends.missingBinding.pct}%
                </span>
              ) : undefined
            }
          />
        ...
        <Col xs={24} sm={12} md={6}>
          <StatCard
            ...
            suffix={
              trends.incidents !== undefined ? (
                <span
                  style={{
                    fontSize: 12,
                    color:
                      trends.incidents.direction === "down"
                        ? "#389e0d"
                        : "#cf1322",
                  }}
                >
                  {trends.incidents.direction === "down" ? "↓" : "↑"}{" "}
                  {trends.incidents.pct}%
                </span>
              ) : undefined
            }
          />
      ...
      <Row gutter={[12, 12]} style={{ marginBottom: 16 }}>
        <Col xs={24} sm={8}>
          <Card size="small" title="Phân bổ trạng thái">
            {(["DRAFT", "READY", "ACTIVE", "DEPRECATED", "ERROR"] as const).map(
              (status) => {
                ...
                return (
                  ...
                  >
                    <Text style={{ width: 72, fontSize: 12 }}>
                      {meta.label}
                    </Text>
                    <div
                      style={{
                    ...
                    <Text
                      strong
                      style={{ width: 28, textAlign: "right", fontSize: 12 }}
                    >
                      {count}
                    </Text>
                  </div>
                      ...
                      <Table
                        ...
                        pagination={{
                          ...
                          showTotal: (total: number) =>
                            `Tổng ${total} config cần chú ý`,
                          hideOnSinglePage: true,
                        }}
                        dataSource={rows.filter(
                          (row) =>
                            row.incidentCount > 0 || row.bindingCount === 0,
                        )}
                  ...
                  <Col xs={24} lg={10}>
                    <Card size="small" title="Loại Service Task">
                      {CATEGORY_GROUPS.map((group) => {
                        const groupTypes = serviceTasks.types.filter(
              ...
              <>
                <Row gutter={[12, 12]} style={{ marginBottom: 12 }}>
                  ...
                  <Col xs={12} sm={6} md={4}>
                    <StatCard
                      ...
                      color={
                        filteredStats.withIncidents ? "#cf1322" : undefined
                      }
                    />
                  </Col>
                  <Col xs={12} sm={6} md={4}>
                    <StatCard
                      ...
                      color={
                        filteredStats.missingBinding ? "#fa8c16" : undefined
                      }
                    />
                  </Col>
                </Row>
                ...
                <Card size="small">
                  <Row
                    gutter={[12, 8]}
                    align="middle"
                    style={{ marginBottom: activeFilters.length > 0 ? 8 : 12 }}
                  >
                    <Col flex="auto">
                      <Space wrap size={8}>
                        <Select
                        ...
                        <Select
                          ...
                          style={{ width: 150 }}
                          value={statusFilter}
                          options={Object.entries(SERVICE_TASK_STATUS_META).map(
                            ([value, meta]) => ({
                              label: meta.label,
                              value,
                            }),
                    ...
                    <Col flex="none">
                      <Space size={4}>
                        ...
                        >
                          <FilterOutlined /> {filteredRows.length}/{rows.length}{" "}
                          cấu hình
                        </Text>
                    ...
                    locale={{
                      emptyText: (
                        <EmptyState
                          ...
                          description={
                            activeFilters.length > 0
                              ? "Thử xóa bộ lọc để xem tất cả cấu hình."
                              : 'Nhấn "Tạo cấu hình" để thêm mới.'
                          }
                        />
                      ),
                    }}
              ...
              <Card size="small">
                ...
                <Table
                  ...
                  columns={[
                    ...
                    {
                      title: "Version",
                      dataIndex: "configVersionNo",
                      width: 90,
                      render: (value) => <Tag>v{value}</Tag>,
                    },
                    {
                      title: "Status",
                      dataIndex: "status",
                      ...
                      columns={[
                        {
                          ...
                          render: (value) => {
                            const definition =
                              serviceTasks.getDefinition(value);
                            return definition ? (
                              <Space direction="vertical" size={0}>
                        ...
                        },
                        {
                          ...
                          render: (_, row) => (
                            <Space direction="vertical" size={0}>
                              {row.before ? (
                                <Text type="secondary">
                                  before: {Object.keys(row.before).join(", ")}
                                </Text>
                              ) : null}
                              {row.after ? (
                                <Text type="secondary">
                                  after: {Object.keys(row.after).join(", ")}
                                </Text>
                              ) : null}
